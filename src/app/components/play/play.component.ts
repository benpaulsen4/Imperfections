...
import {
  DataSource,
  DataSourceToken,
} from '../../services/data-source.interface';
...
export class PlayComponent implements OnInit, OnDestroy {
  ...
  private getNewOptions() {
    ...
    timer(500)
      ...
      .subscribe(() => this.character1.next(char1));
    timer(1000)
      .pipe(takeUntil(this.cancelled$))
      .subscribe(() => this.character2.next(char2));
    timer(1500)
      .pipe(takeUntil(this.cancelled$))
      .subscribe(() => this.character3.next(char3));

    timer(3500)
      .pipe(takeUntil(this.cancelled$))
      .subscribe(() => this.flag1.next(flag1));
    timer(4000)
      .pipe(takeUntil(this.cancelled$))
      .subscribe(() => this.flag2.next(flag2));
    timer(4500)
      .pipe(takeUntil(this.cancelled$))
      .subscribe(() => this.flag3.next(flag3));
  }
