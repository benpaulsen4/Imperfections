...
export class HostedDataSourceService implements DataSource {
  ...
  async cacheData(): Promise<void> {
    ...
    if (
      this.redFlags.getValue().length > 0 &&
      this.characters.getValue().length > 0
    )
      return;
    ...
    if (!!responses.find((res) => res.status >= 400))
      throw new Error('Data fetch failed');

    this.redFlags.next(await responses[0].json());

    const characters = ((await responses[1].json()) as Character[]).map(
      (character) => {
        const imageFile = character.imgUrl;

        if (!!imageFile) {
          character.imgUrl =
            environment.hostedDataSourceBaseUrl +
            this.imagesPathExtension +
            imageFile;
        }
  ...
  getThreeCharacters(): Character[] {
    ...
    if (cachedCharacters.length === 0)
      throw new Error('Character set not initialized');
  ...
  getThreeFlags(): string[] {
    ...
    if (cachedFlags.length === 0)
      throw new Error('Red flag set not initialized');
